...
(async () => {
	try {
		const [todo, user] = await Promise.all([TodoService.getById(1), UserService.getById(1)]);
		console.log('Todo:', todo);
		console.log('User:', user);

		const first = await Promise.race([TodoService.getById(1), UserService.getById(1)]);
